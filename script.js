...
(() => {
  ...
  const repositoryAssignments = {
    ...
    "crowdcheck-s4-moderate-reliability": {
      "study": 4,
      "condition": "moderate_reliability"
    },
    "crowdcheck-s4-high-reliability": {
      "study": 4,
      "condition": "high_reliability"
    }
  };
  ...
  const conditions = {
    ...
    4: ["control", "moderate_reliability", "high_reliability"]
  };
const state = {
    participantId,
    sessionId,
  ...
  function noteMetadata(condition) {
    if (condition === "few_sources") {
      return "评价信息：5名社区用户分别核查了这篇帖子，其中4人支持这条附注。";
    }
    if (condition === "many_sources") {
      return "评价信息：30名社区用户分别核查了这篇帖子，其中24人支持这条附注。";
    }
    if (condition === "moderate_reliability") {
      return "评价信息：30名社区用户分别核查了这篇帖子，其中24人支持这条附注。";
    }
    if (condition === "high_reliability") {
      return "评价信息：30名社区用户分别核查了这篇帖子，其中24人支持这条附注。";
    }
    return "";
  }
  ...
  function renderCommunityNote(){const hasNote=state.eligibleForRandomization&&state.condition!=="control"&&state.postDeclaration!=="ai";$("community-note").hidden=!hasNote;if(!hasNote)return;$("community-note").classList.remove("resolved");$("note-heading-text").textContent="社区附注 · 公开";$("note-primary-text").textContent="社区核查结果：这篇帖子的文案使用了生成式AI，但发布者尚未添加AI内容声明。";$("note-secondary-text").textContent="这条附注已公开显示，所有浏览者均可见。";}
  ...
  function finishExperiment(path) {
    ...
    const storageKey = `crowdchecking-study3-${sessionId}`;
    try {
      localStorage.setItem(storageKey, JSON.stringify(state));
    } catch (_) {
      state.storageWarning = true;
    }
    window.parent?.postMessage({ type: "crowdchecking-study3-update", payload: state }, "*");
...
$("consent").addEventListener("change", (event) => {
  ...
  logEvent("experiment_loaded", {
    study,
    participantId,
    forcedCondition: fixedCondition || null,
